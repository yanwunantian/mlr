...
#' @rdname Task
#' @family costsens
makeCostSensTask = function(id = deparse(substitute(data)), data, costs, blocking = NULL, check.data = TRUE) {
  assertString(id)
  ...
  task = makeSupervisedTask("costsens", data, target, weights, blocking, check.data = check.data)
